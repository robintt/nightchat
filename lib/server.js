...
var handlers = [
	...
	{ path: "/join", handler: function(channel, request, response) {
		...
		if (!nick) {
			response.simpleJSON(400, { error: "zły login." });
			return;
		}
		var session = channel.createSession(nick);
		if (!session) {
			response.simpleJSON(400, { error: "login w użyciu." });
			return;
		}
	...
	{ path: "/recv", handler: function(channel, request, response) {
		...
		if (!session) {
			response.simpleJSON(400, { error: "Nie ma takiego identyfikatora sesji." });
			return;
		}
		
		if (isNaN(since)) {
			response.simpleJSON(400, { error: "Musi dostarczyc od parametru." });
			return;
		}
	...
	{ path: "/send", handler: function(channel, request, response) {
		...
		if (!text || !text.length) {
			response.simpleJSON(400, { error: "Musi dostarczyc parametr tekstowy." });
			return;
		}
